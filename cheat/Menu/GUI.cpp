...
void CWindow::Paint(int t_x, int t_y, float* controlheight)
{
	...
	Draw::FilledRectangle(tl.x, tl.y - 16, br.x, br.y, Color(0, 68, 100, 135));//midden
	Draw::Rectangle(tl.x, tl.y - 16, br.x, br.y, Color(5, 36, 50));//buitenste ring
	Draw::Rectangle(tl.x + 1, tl.y - 15, br.x - 1, br.y - 1, Color(10, 140, 200));//buitenste ring midden
	tl += 6;
	br -= 6;
	Draw::FilledGradientRectangle(tl.x, tl.y, br.x, br.y, Color(238, 238, 238), Color(210, 210, 210));
	Draw::Rectangle(tl, br, Color(10, 140, 200));//middenste ring midden
	Draw::Rectangle(tl.x - 1, tl.y - 1, br.x + 1, br.y + 1, Color(5, 36, 50));//middenste ring buiten

	Draw::Text(tl.x + 3, tl.y - 17, L"daswaut.xyz - best cheetos", Font::Get().MenuText, Color::White);
...
void CCheckBox::Paint(int t_x, int t_y, float* controlheight)
{
	...
	if (m_val != nullptr && *m_val)
		Draw::Text(tl.x + (size / 2) + 1, tl.y + (size / 2), L"✔", Font::Get().MenuText, Color(10, 140, 200), true); //✔
...
inline void CSlider<T>::Paint(int t_x, int t_y, float* controlheight)
{
	...
	Draw::FilledRectangle(tl.x, tl.y + 3, valueX, br.y - 3, Color(10, 140, 200));
